 ...
 *   --games=drift-boss,drive-mad  Restrict the global catalog, site games and
 *                                 category graph to these game keys.
 *   --include-posts=false          Skip legacy blog/article export (useful for
 *                                 a deliberately small Preview dataset).
...
const includePosts = (args.get('include-posts') || 'true').toLowerCase() !== 'false';
...
try {
  ...
  const availableGameKeys = new Set(
    legacyGames.map((row: any) => String(row.game_key || '').trim()).filter(Boolean)
  );
  const missingGameKeys = [...selectedGameKeys].filter((key) => !availableGameKeys.has(key));
  if (missingGameKeys.length) {
  ...
  const selectedGames = selectedGameKeys.size
  ...
  const gameByKey = new Map(selectedGames.map((row: any) => [String(row.game_key), row]));
  const categoryByKey = new Map(selectedCategories.map((row: any) => [String(row.category), row]));
  const output: string[] = [
