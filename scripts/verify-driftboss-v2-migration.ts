...
const databaseUrl = process.env.DATABASE_URL || 'file:data/legacy-migration-target.db';
const client = createClient({ url: databaseUrl });

async function one<T extends Record<string, unknown>>(sql: string, args: unknown[] = []) {
...
try {
  const siteCount = await one<{ c: number }>('SELECT COUNT(*) AS c FROM game_site');
  ...
  expect(Number(catalog?.c) === 1, 'drift-boss must exist once in global catalog');
  ...
  expect(Number(siteGames?.c) === 2, 'drift-boss must have two site_game rows');
  ...
  const gameA = await one<{ slug: string; content: string; meta_description: string }>(
    `SELECT sgl.slug, sgl.content, sgl.meta_description
       FROM site_game_locale sgl
      WHERE sgl.site_id = ? AND sgl.locale = 'en'`,
    [siteA.id]
  );
  const gameB = await one<{ slug: string; content: string; meta_description: string }>(
    `SELECT sgl.slug, sgl.content, sgl.meta_description
       FROM site_game_locale sgl
      WHERE sgl.site_id = ? AND sgl.locale = 'en'`,
    [siteB.id]
  ...
  expect(gameA?.slug === 'drift-boss', `unexpected Site A slug: ${gameA?.slug}`);
  expect(
  ...
  expect(homeA?.meta_title === 'Drift Boss Meta A', 'Site A homepage meta missing');
  expect(homeA?.content === 'Unique homepage content A', 'Site A homepage body missing');
  expect(homeB?.meta_title === 'Drift Boss Meta B', 'Site B homepage meta missing');
  expect(homeB?.content === 'Unique homepage content B', 'Site B homepage body missing');

  const aboutA = await one<{ content: string }>(
    `SELECT spl.content
       FROM site_post_locale spl
       JOIN site_post sp ON sp.id = spl.site_post_id
      WHERE sp.site_id = ? AND sp.type = 'page' AND spl.slug = 'about-us'`,
    [siteA.id]
  );
  const aboutB = await one<{ content: string }>(
    `SELECT spl.content
       FROM site_post_locale spl
       JOIN site_post sp ON sp.id = spl.site_post_id
      WHERE sp.site_id = ? AND sp.type = 'page' AND spl.slug = 'about-us'`,
  ...
  expect(aboutA?.content === 'About Drift A', 'Site A About page missing');
  expect(aboutB?.content === 'About Drift B', 'Site B About page missing');
  ...
  expect(socialA?.value?.includes('YouTube'), 'Site A social links were not migrated');
  expect(Number(socialB?.c) === 0, 'Site B should not receive Site A social links');
  ...
  expect(blogA?.content?.includes('Site A guide-like article'), 'Legacy blog missing');
